...
sudo cp "$BASE_DIR/backend_fastapi/deploy/photopolymer-api.service" /etc/systemd/system/
echo "✓ 后端服务配置已更新"
echo "  源文件: $BASE_DIR/backend_fastapi/deploy/photopolymer-api.service"
echo "  目标位置: /etc/systemd/system/photopolymer-api.service"
echo ""

# 3. 更新前端 Nginx 配置
echo -e "${GREEN}[3/6] 更新前端 Nginx 配置...${NC}"
sudo cp "$BASE_DIR/frontend_vue3/deploy/photopolymer-frontend.conf" /etc/nginx/sites-available/photopolymer
sudo ln -sf /etc/nginx/sites-available/photopolymer /etc/nginx/sites-enabled/photopolymer
echo "✓ Nginx 配置已更新"
echo "  源文件: $BASE_DIR/frontend_vue3/deploy/photopolymer-frontend.conf"
