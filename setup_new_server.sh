...
cp "$SCRIPT_DIR/backend_fastapi/depoly/photopolymer-api.service" \
   "$SCRIPT_DIR/backend_fastapi/depoly/photopolymer-api.service.backup.$(date +%Y%m%d_%H%M%S)"
cp "$SCRIPT_DIR/frontend_vue3/depoly/photopolymer-frontend.conf" \
   "$SCRIPT_DIR/frontend_vue3/depoly/photopolymer-frontend.conf.backup.$(date +%Y%m%d_%H%M%S)"
echo "✓ 备份完成"
echo ""

# 修改后端服务配置
echo -e "${YELLOW}[2/4] 修改后端服务配置...${NC}"
BACKEND_SERVICE="$SCRIPT_DIR/backend_fastapi/depoly/photopolymer-api.service"
...
NGINX_CONF="$SCRIPT_DIR/frontend_vue3/depoly/photopolymer-frontend.conf"
...
echo ""
echo "5. 查看备份文件："
echo "   ls -la $SCRIPT_DIR/backend_fastapi/depoly/*.backup.*"
echo "   ls -la $SCRIPT_DIR/frontend_vue3/depoly/*.backup.*"
echo ""
