# PhotoPolymer 配方数据库 - 部署说明

## 📋 前置条件

### 1. Python虚拟环境
```bash
# 虚拟环境必须位于：~/venv/database/
# 如果不存在，创建它：
python3 -m venv ~/venv/database
source ~/venv/database/bin/activate
cd backend_fastapi
pip install -r requirements.txt
```

### 2. 前端构建
```bash
cd frontend_vue3
pnpm install
pnpm run build
```

### 3. 文件权限
```bash
# 确保nginx可以访问项目目录
chmod 711 /home/xgs
chmod 755 /home/xgs/workspace
chmod 755 /home/xgs/workspace/database
chmod 755 /home/xgs/workspace/database/data_base
chmod 755 /home/xgs/workspace/database/data_base/frontend_vue3
chmod -R 755 /home/xgs/workspace/database/data_base/frontend_vue3/dist
```

---

## 🚀 一键部署

```bash
cd /home/xgs/workspace/database/data_base
bash deploy.sh
```

或使用自动安装nginx的脚本：

```bash
bash install_and_deploy.sh
```

---

## 📍 访问地址

部署成功后：

- **前端应用**: http://localhost:8080
- **后端API**: http://localhost:8000
- **API文档**: http://localhost:8000/docs

---

## ⚠️ 常见问题

### 1. 端口8000被占用

如果有其他服务（如mineru-api）占用8000端口：

```bash
# 停止占用进程
sudo pkill -f mineru-api

# 重启后端服务
sudo systemctl restart photopolymer-api
```

### 2. Nginx 403 Forbidden

检查目录权限（参见"文件权限"部分）。

### 3. 登录404错误

- 检查后端服务是否运行：`sudo systemctl status photopolymer-api`
- 查看日志：`sudo journalctl -u photopolymer-api -f`

### 4. 前端JS错误（循环依赖）

清除缓存并重新构建：

```bash
cd frontend_vue3
rm -rf dist node_modules/.vite
pnpm run build
chmod -R 755 dist
sudo systemctl reload nginx
```

浏览器按 `Ctrl+Shift+R` 强制刷新。

---

## 🔧 服务管理

### 查看状态
```bash
sudo systemctl status photopolymer-api
sudo systemctl status nginx
```

### 启动/停止/重启
```bash
# 后端
sudo systemctl start photopolymer-api
sudo systemctl stop photopolymer-api
sudo systemctl restart photopolymer-api

# 前端
sudo systemctl restart nginx
```

### 查看日志
```bash
# 后端实时日志
sudo journalctl -u photopolymer-api -f

# 前端访问日志
sudo tail -f /var/log/nginx/photopolymer-access.log

# 前端错误日志
sudo tail -f /var/log/nginx/photopolymer-error.log
```

---

## 📝 配置文件位置

- **后端服务**: `backend_fastapi/depoly/photopolymer-api.service`
- **前端Nginx**: `frontend_vue3/depoly/photopolymer-frontend.conf`
- **部署脚本**: `deploy.sh` 或 `install_and_deploy.sh`

---

## 💡 重要提示

1. ✅ 虚拟环境路径：`~/venv/database/` （不在项目目录内）
2. ✅ 后端启动命令：`python main.py --env=prod` （没有`run`参数）
3. ✅ 前端端口：8080 （避免需要root权限）
4. ✅ 缓存策略：index.html不缓存，JS/CSS长期缓存

