# PhotoPolymer 配方数据库 - 完整部署指南

本文档提供从零开始在新服务器上部署系统的完整步骤。

---

## 📋 准备工作：克隆代码

```bash
# 1. 进入工作目录（根据实际情况修改）
cd /home/xgs/workspace

# 2. 克隆代码
git clone https://github.com/shuo1104/database-bate-0.3.git
cd database-bate-0.3
```

**⚠️ 重要提示**：
- 记录您的项目路径，后续配置需要使用
- 示例：`/home/xgs/workspace/database-bate-0.3`

---

## 第一部分：安装系统依赖

### 1. 安装系统依赖

```bash
# 更新包列表
sudo apt update

# 安装 PostgreSQL 数据库
sudo apt install postgresql postgresql-contrib -y

# 安装 Nginx
sudo apt install nginx -y

# 安装 Python 虚拟环境工具
sudo apt install python3-venv python3-pip -y

# 安装 Node.js 和 pnpm
curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -
sudo apt install nodejs -y
npm install -g pnpm
```

### 2. 创建 Python 虚拟环境

```bash
# 创建虚拟环境目录
mkdir -p ~/venv

# 创建虚拟环境
python3 -m venv ~/venv/database

# 激活虚拟环境
source ~/venv/database/bin/activate

# 安装后端依赖（注意：没有 data_base 目录）
cd ~/workspace/database-bate-0.3/backend_fastapi
pip install -r requirements.txt
```

**⚠️ 路径说明**：
- ✅ 正确：`~/workspace/database-bate-0.3/backend_fastapi`
- ❌ 错误：`~/workspace/data_base/database-bate-0.3/backend_fastapi`

### 3. 安装前端依赖

```bash
# 进入前端目录
cd ~/workspace/database-bate-0.3/frontend_vue3
pnpm install
```

---

## 第二部分：配置路径（新服务器必须执行）

### 自动配置（推荐）

```bash
# 返回项目根目录
cd ~/workspace/database-bate-0.3

# 运行自动配置脚本
bash setup_new_server.sh
```

脚本会询问：
- 用户名（默认：当前用户）
- 用户组（默认：当前用户组）
- 项目路径（默认：当前目录）
- 虚拟环境路径（默认：~/venv/database）
- 后端端口（默认：8000）
- 前端端口（默认：8080）

### 手动配置（可选）

如果不使用自动脚本，需要手动修改以下文件：

**1. 修改后端服务配置**：
```bash
nano backend_fastapi/deploy/photopolymer-api.service
```

将所有路径中的：
- `xgs` → 替换为您的用户名
- `/home/xgs/workspace/database-bate-0.3` → 替换为您的项目路径
- `/home/xgs/venv/database` → 替换为您的虚拟环境路径

**2. 修改前端 Nginx 配置**：
```bash
nano frontend_vue3/deploy/photopolymer-frontend.conf
```

将 `root` 路径修改为您的前端构建目录。

---

## 第三部分：配置数据库

### 1. 启动 PostgreSQL

```bash
sudo systemctl start postgresql
sudo systemctl enable postgresql
```

### 2. 创建数据库和用户

```bash
# 切换到 postgres 用户
sudo -u postgres psql
```

在 psql 中执行以下命令：

```sql
CREATE DATABASE photopolymer_formulation_db;
CREATE USER photopolymer_admin WITH PASSWORD 'root';
GRANT ALL PRIVILEGES ON DATABASE photopolymer_formulation_db TO photopolymer_admin;
\q
```

**⚠️ 安全提示**：
- 生产环境请使用强密码替换 `'root'`
- 记录数据库名、用户名和密码，后续配置需要使用

### 3. 配置后端数据库连接（如果需要）

如果项目有独立的环境配置文件：

```bash
cd ~/workspace/database-bate-0.3/backend_fastapi

# 检查是否存在环境配置文件
ls env/.env.prod

# 如果存在，编辑配置
nano env/.env.prod
```

修改数据库配置：
```env
DB_HOST=localhost
DB_PORT=5432
DB_NAME=photopolymer_formulation_db
DB_USER=photopolymer_admin
DB_PASSWORD=root
```

---

## 第四部分：构建前端

```bash
cd ~/workspace/database-bate-0.3/frontend_vue3
pnpm build
```

**验证构建**：
```bash
ls -la dist/
```

应该看到 `index.html` 和其他静态文件。

---

## 第五部分：部署服务

### 使用一键部署脚本（推荐）

```bash
cd ~/workspace/database-bate-0.3
bash update_configs.sh
```

脚本会自动：
1. 停止现有服务
2. 复制配置文件到系统目录
3. 测试 Nginx 配置
4. 重新加载服务
5. 启动所有服务
6. 显示服务状态

### 手动部署（可选）

如果不使用自动脚本：

**1. 部署后端服务**：
```bash
sudo cp backend_fastapi/deploy/photopolymer-api.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl start photopolymer-api.service
sudo systemctl enable photopolymer-api.service
```

**2. 部署前端 Nginx**：
```bash
sudo cp frontend_vue3/deploy/photopolymer-frontend.conf /etc/nginx/sites-available/photopolymer
sudo ln -sf /etc/nginx/sites-available/photopolymer /etc/nginx/sites-enabled/photopolymer
sudo nginx -t
sudo systemctl reload nginx
sudo systemctl enable nginx
```

---

## 第六部分：验证部署

### 1. 检查所有服务状态

```bash
# 检查数据库
systemctl status postgresql

# 检查后端
systemctl status photopolymer-api.service

# 检查 Nginx
systemctl status nginx
```

所有服务应该显示 `active (running)`。

### 2. 检查端口监听

```bash
# 检查后端端口 8000
sudo lsof -i :8000

# 检查 Nginx 端口 8080
sudo lsof -i :8080

# 检查数据库端口 5432
sudo lsof -i :5432
```

### 3. 测试访问

```bash
# 测试后端 API
curl http://localhost:8000/docs

# 测试前端
curl http://localhost:8080
```

### 4. 浏览器访问

打开浏览器访问：
- 前端应用：`http://YOUR_SERVER_IP:8080`
- 后端文档：`http://YOUR_SERVER_IP:8000/docs`

---

## 第七部分：防火墙配置（如果需要）

```bash
# 允许 8080 端口（前端）
sudo ufw allow 8080/tcp

# 允许 SSH（重要！避免被锁定）
sudo ufw allow 22/tcp

# 启用防火墙
sudo ufw enable

# 查看状态
sudo ufw status
```

---

## 📝 故障排查

### 后端启动失败

```bash
# 查看详细日志
sudo journalctl -u photopolymer-api.service -n 100 --no-pager

# 常见错误：
# - "CHDIR failed" → 路径配置错误，检查 photopolymer-api.service
# - "ModuleNotFoundError" → 依赖未安装，重新运行 pip install
# - "Database connection failed" → 数据库配置错误

# 手动测试启动
source ~/venv/database/bin/activate
cd ~/workspace/database-bate-0.3/backend_fastapi
python main.py --env=prod
```

### Nginx 启动失败

```bash
# 测试配置
sudo nginx -t

# 常见错误：
# - "No such file or directory" → 前端 dist 目录不存在，需要先构建
# - "Permission denied" → 权限问题，检查目录权限

# 查看错误日志
sudo tail -f /var/log/nginx/error.log
```

### 数据库连接失败

```bash
# 检查 PostgreSQL 状态
systemctl status postgresql

# 测试连接
psql -U photopolymer_admin -d photopolymer_formulation_db -h localhost

# 如果连接失败，检查：
# 1. 数据库是否创建
# 2. 用户是否创建
# 3. 密码是否正确
```

### 前端 403 Forbidden

```bash
# 设置正确的目录权限
chmod 755 ~
chmod 755 ~/workspace
chmod -R 755 ~/workspace/database-bate-0.3/frontend_vue3/dist
```

---

## 🔄 完整启动顺序

```bash
# 1. 启动数据库
sudo systemctl start postgresql

# 2. 启动后端
sudo systemctl start photopolymer-api.service

# 3. 启动 Nginx
sudo systemctl start nginx

# 4. 检查所有服务
systemctl status postgresql photopolymer-api.service nginx
```

---

## 🛑 停止所有服务

```bash
# 使用停止脚本
bash stop_system.sh

# 或手动停止
sudo systemctl stop photopolymer-api.service
sudo systemctl stop nginx
sudo systemctl stop postgresql
```

---

**部署完成！** 🎉

如有问题，请查看：
- 详细路径配置：`doc/路径配置说明.md`
- 新服务器部署：`doc/新服务器部署路径修改清单.md`
- 快速参考：`新服务器部署快速参考.md`
