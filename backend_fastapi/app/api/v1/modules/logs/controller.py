...
from .schema import LoginLogListQuery, RegistrationLogListQuery, DailyUsageListQuery
...
async def get_daily_usage_statistics(
    ...
    - 每日总使用时长和平均使用时长
    """
    query = DailyUsageListQuery(days=days, start_date=start_date, end_date=end_date)
...
async def update_heartbeat(
    ...
    - 前端应该每1-5分钟调用一次
    """
    success = await LogService.update_heartbeat(db, log_id, user_id=current_user_id)
...
async def user_logout(
    ...
    - 计算使用时长
    """
    success = await LogService.record_logout(db, log_id, user_id=current_user_id)
