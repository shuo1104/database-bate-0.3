...
class LogCRUD:
    ...
    async def get_or_create_system_info(db: AsyncSession) -> SystemInfoModel:
        ...
        if not system_info:
            system_info = SystemInfoModel(
                FirstStartTime=datetime.now(), Version="1.0.0"
            )
            db.add(system_info)
            await db.commit()
    ...
    async def update_logout_log(
        ...
        if user_id is not None:
            stmt = stmt.where(UserLoginLogModel.UserID == user_id)
        result = await db.execute(stmt)
    ...
    async def get_system_statistics(db: AsyncSession) -> dict:
        ...
        active_users_stmt = select(
            func.count(distinct(UserLoginLogModel.UserID))
        ).where(
            and_(
                UserLoginLogModel.LoginTime >= today_start,
                UserLoginLogModel.LoginTime < today_end,
    ...
    async def get_daily_usage_statistics(
        ...
        stmt = (
            ...
            .group_by(func.date(UserLoginLogModel.LoginTime))
            .order_by(func.date(UserLoginLogModel.LoginTime).desc())
        )
