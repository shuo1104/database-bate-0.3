...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    if not hashed_password or not hashed_password.startswith("$"):
        logger.error(
            f"Security Warning: Illegal password format detected, verification refused"
        )
        raise ValueError(
    ...
    try:
        ...
        # 其他未预期的错误
        logger.error(
            f"Password verification failed - Unknown error: {type(e).__name__}: {e}",
            exc_info=True,
        )
        return False
...
def decode_token(token: str, required_type: Optional[str] = "access") -> dict[str, Any]:
    """
    解码JWT令牌
...
async def get_current_user_id(
    ...
    payload = decode_token(token, required_type="access")
