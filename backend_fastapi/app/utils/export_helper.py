...
class ExportHelper:
    ...
    def sanitize_spreadsheet_value(value: Any) -> Any:
        ...
        if stripped and stripped[0] in ExportHelper.DANGEROUS_SPREADSHEET_PREFIXES:
    ...
    @staticmethod
    def export_to_csv(
        data: List[Dict[str, Any]], columns: List[str], filename: str = None
    ) -> tuple[bytes, str]:
        """
        导出数据为CSV格式
        ...
        for row in data:
            ...
            sanitized_row = {
                k: ExportHelper.sanitize_spreadsheet_value(v)
    ...
    def export_to_txt(
        ...
        for row in data:
            ...
            for col in columns:
                value = row.get(col, "")
                value = ExportHelper.sanitize_spreadsheet_value(value)
    ...
    def prepare_export_data(
        items: List[Any], column_mapping: Dict[str, str] = None
    ) -> List[Dict[str, Any]]:
        """
        准备导出数据
