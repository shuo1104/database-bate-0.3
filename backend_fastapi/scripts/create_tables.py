...
TABLES["fn_validate_project_type_for_test"] = (
    'CREATE OR REPLACE FUNCTION "fn_validate_project_type_for_test"() '
    "RETURNS TRIGGER AS $$ "
    "DECLARE actual_type_code VARCHAR(10); "
    "BEGIN "
    '  SELECT pt."TypeCode" INTO actual_type_code '
    '  FROM "tbl_ProjectInfo" p '
    '  JOIN "tbl_Config_ProjectTypes" pt ON p."ProjectType_FK" = pt."TypeID" '
    '  WHERE p."ProjectID" = NEW."ProjectID_FK"; '
    "  IF actual_type_code IS NULL THEN "
    "    RAISE EXCEPTION 'ProjectID % has no ProjectType', NEW.\"ProjectID_FK\"; "
    "  END IF; "
    "  IF actual_type_code <> TG_ARGV[0] THEN "
    "    RAISE EXCEPTION 'ProjectID % has type %, cannot write to %', NEW.\"ProjectID_FK\", actual_type_code, TG_TABLE_NAME; "
    "  END IF; "
    "  RETURN NEW; "
    "END; "
    "$$ LANGUAGE plpgsql; "
...
TABLES["fn_validate_project_type_change"] = (
    'CREATE OR REPLACE FUNCTION "fn_validate_project_type_change"() '
    "RETURNS TRIGGER AS $$ "
    "DECLARE new_type_code VARCHAR(10); "
    "BEGIN "
    '  IF NEW."ProjectType_FK" IS NULL THEN '
    '    IF EXISTS (SELECT 1 FROM "tbl_TestResults_Ink" WHERE "ProjectID_FK" = NEW."ProjectID") '
    '       OR EXISTS (SELECT 1 FROM "tbl_TestResults_Coating" WHERE "ProjectID_FK" = NEW."ProjectID") '
    '       OR EXISTS (SELECT 1 FROM "tbl_TestResults_3DPrint" WHERE "ProjectID_FK" = NEW."ProjectID") '
    '       OR EXISTS (SELECT 1 FROM "tbl_TestResults_Composite" WHERE "ProjectID_FK" = NEW."ProjectID") THEN '
    "      RAISE EXCEPTION 'ProjectID % has test results; ProjectType cannot be NULL', NEW.\"ProjectID\"; "
    "    END IF; "
    "    RETURN NEW; "
    "  END IF; "
    '  SELECT "TypeCode" INTO new_type_code '
    '  FROM "tbl_Config_ProjectTypes" '
    '  WHERE "TypeID" = NEW."ProjectType_FK"; '
    "  IF new_type_code IS NULL THEN "
    '    RAISE EXCEPTION \'Invalid ProjectType_FK % for ProjectID %\', NEW."ProjectType_FK", NEW."ProjectID"; '
    "  END IF; "
    '  IF new_type_code <> \'INK\' AND EXISTS (SELECT 1 FROM "tbl_TestResults_Ink" WHERE "ProjectID_FK" = NEW."ProjectID") THEN '
    "    RAISE EXCEPTION 'ProjectID % already has Ink test results; cannot change type to %', NEW.\"ProjectID\", new_type_code; "
    "  END IF; "
    '  IF new_type_code <> \'COAT\' AND EXISTS (SELECT 1 FROM "tbl_TestResults_Coating" WHERE "ProjectID_FK" = NEW."ProjectID") THEN '
    "    RAISE EXCEPTION 'ProjectID % already has Coating test results; cannot change type to %', NEW.\"ProjectID\", new_type_code; "
    "  END IF; "
    '  IF new_type_code <> \'3DP\' AND EXISTS (SELECT 1 FROM "tbl_TestResults_3DPrint" WHERE "ProjectID_FK" = NEW."ProjectID") THEN '
    "    RAISE EXCEPTION 'ProjectID % already has 3DPrint test results; cannot change type to %', NEW.\"ProjectID\", new_type_code; "
    "  END IF; "
    '  IF new_type_code <> \'COMP\' AND EXISTS (SELECT 1 FROM "tbl_TestResults_Composite" WHERE "ProjectID_FK" = NEW."ProjectID") THEN '
    "    RAISE EXCEPTION 'ProjectID % already has Composite test results; cannot change type to %', NEW.\"ProjectID\", new_type_code; "
    "  END IF; "
    "  RETURN NEW; "
    "END; "
    "$$ LANGUAGE plpgsql; "
...
TABLES["trg_TestResults_Ink_ProjectType"] = (
    'DROP TRIGGER IF EXISTS "trg_testresults_ink_project_type" ON "tbl_TestResults_Ink"; '
    'CREATE TRIGGER "trg_testresults_ink_project_type" '
    'BEFORE INSERT OR UPDATE ON "tbl_TestResults_Ink" '
    "FOR EACH ROW EXECUTE FUNCTION \"fn_validate_project_type_for_test\"('INK'); "
)

TABLES["trg_TestResults_Coating_ProjectType"] = (
    'DROP TRIGGER IF EXISTS "trg_testresults_coating_project_type" ON "tbl_TestResults_Coating"; '
    'CREATE TRIGGER "trg_testresults_coating_project_type" '
    'BEFORE INSERT OR UPDATE ON "tbl_TestResults_Coating" '
    "FOR EACH ROW EXECUTE FUNCTION \"fn_validate_project_type_for_test\"('COAT'); "
)

TABLES["trg_TestResults_3DPrint_ProjectType"] = (
    'DROP TRIGGER IF EXISTS "trg_testresults_3dprint_project_type" ON "tbl_TestResults_3DPrint"; '
    'CREATE TRIGGER "trg_testresults_3dprint_project_type" '
    'BEFORE INSERT OR UPDATE ON "tbl_TestResults_3DPrint" '
    "FOR EACH ROW EXECUTE FUNCTION \"fn_validate_project_type_for_test\"('3DP'); "
)

TABLES["trg_TestResults_Composite_ProjectType"] = (
    'DROP TRIGGER IF EXISTS "trg_testresults_composite_project_type" ON "tbl_TestResults_Composite"; '
    'CREATE TRIGGER "trg_testresults_composite_project_type" '
    'BEFORE INSERT OR UPDATE ON "tbl_TestResults_Composite" '
    "FOR EACH ROW EXECUTE FUNCTION \"fn_validate_project_type_for_test\"('COMP'); "
)

TABLES["trg_ProjectInfo_ProjectType_Change"] = (
    'DROP TRIGGER IF EXISTS "trg_projectinfo_project_type_change" ON "tbl_ProjectInfo"; '
    'CREATE TRIGGER "trg_projectinfo_project_type_change" '
    'BEFORE UPDATE OF "ProjectType_FK" ON "tbl_ProjectInfo" '
    'FOR EACH ROW EXECUTE FUNCTION "fn_validate_project_type_change"(); '
)
...
TABLE_ORDER = [
    ...
    "fn_validate_project_type_for_test",
    "fn_validate_project_type_change",
    "trg_TestResults_Ink_ProjectType",
    "trg_TestResults_Coating_ProjectType",
    "trg_TestResults_3DPrint_ProjectType",
    "trg_TestResults_Composite_ProjectType",
    "trg_ProjectInfo_ProjectType_Change",
    "tbl_SystemInfo",
    "tbl_UserLoginLogs",
...
def main():
    ...
    try:
        cursor.execute(
            'SELECT COUNT(*) FROM "tbl_Users" WHERE "Username" = %s', ("admin",)
        )
        if cursor.fetchone()[0] == 0:
            try:
